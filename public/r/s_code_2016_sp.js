...
var s_account="rcrtrikunabi2016spdev"
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.dynamicAccountSelection=true
s.dynamicAccountMatch=window.location.host+window.location.pathname
s.dynamicAccountList="rcrtrikunabi2016spprd=job.rikunabi.com/2016,job.rikunabi.com/event,job.rikunabi.com/kaigai,arorua.net"
s.charSet="UTF-8"
...
s.linkInternalFilters="javascript:,job.rikunabi.com,arorua.net"
...
function s_doPlugins(s) {
	...
	* リクナビ2013以来計測を開始したもの
	*****************************************/
	if(s.pageName && (tempValue = s.getQueryParam('pn'))){
		s.pageName += ':' + tempValue;
		s.eVar1 = s.pageName;
	}
	if(s.pageName && (tempValue = s.getQueryParam('themeName'))){
		s.pageName += ':' + tempValue.replace(/\//g, ':');
		s.eVar1 = s.pageName;
	}
	if(s.pageName && (tempValue = s.getQueryParam('contents'))){
		s.pageName += ':' + tempValue.replace(/\//g, ':');
		s.eVar1 = s.pageName;
	}
	if(s.pageName && (tempValue = s.getQueryParam('dir'))){
		s.pageName += ':' + tempValue.replace(/\//g, ':');
		s.eVar1 = s.pageName;
	}
	if(s.pageName && (tempValue = s.getQueryParam('file'))){
		s.pageName += ':' + tempValue.replace(/\//g, ':');
		s.eVar1 = s.pageName;
	}

	/* 2012.10.29 themeCd */
	if(s.pageName && (tempValue = s.getQueryParam('themeCd'))){
		s.pageName += ':' + tempValue;
		s.eVar1 = s.pageName;
	}
	s.pageName = s.pageName  + ":sp";
	s.eVar1 = s.pageName;
	...
	* 検索パラメータの取得
	*****************************************/
	s.prop14=s.getQueryParam('kdb');
	/****************************************
	* 内部流入パラメータの取得
	* 
	* 特定ページにてリセット（値を「TOP」等に
	* 設定）されていなければGETパラメータ値を
	* 取得
	*****************************************/
	var sc_myURL = location.pathname;
	if(sc_myURL.match(/^\/2016\/+($|;|\?)/)){
		/* トップ */
		s.prop18 = "TOP";
	} else if (sc_myURL.match(/^\/2016\/search\/pre\/internship\/+($|;|\?)/)) {
		/* インターントップ */
		s.prop18 = "TOP";
	} else if (sc_myURL.match(/^\/2016\/search\/pre\/company\/+($|;|\?)/)) {
		/* 企業研究トップ */
		s.prop18 = "TOP";
	} else if (sc_myURL.match(/^\/2016\/search\/company\/+($|;|\?)/)) {
		/* 企業検索トップ */
		s.prop18 = "TOP";
	} else if (sc_myURL.match(/^\/2016\/mypage\/bookmark\/+($|;|\?)/)) {
		/* マイページ */
		s.prop18 = "TOP";
	} else if (sc_myURL.match(/^\/2016\/contents\/event\/+$/)) {
		var myGETParams = location.search;
		if (myGETParams.match(/\?sideMenu=menu&contents=index/)) {
			/* イベントトップ */
			s.prop18 = "TOP";
		}
	} else if (sc_myURL.match(/^\/2016\/contents\/useful\/+($|;|\?)/)) {
		/* お役立ちトップ */
		s.prop18 = "TOP";
	}
	if (!s.prop18)
		s.prop18=s.getQueryParam('isc');
	/****************************************
	...
	*****************************************/
	/* prop21/eVar21 大業種 */
	s.prop21 = s.getQueryParam('bbc');
	s.eVar21 = s.prop21 ? 'D=c21' : null;
	/* prop22/eVar22 大業種01 */
	s.prop22 = s.getQueryParam('bbc01');
	s.eVar22 = s.prop22 ? 'D=c22' : null;
	/* prop23/eVar23 小業種 */
	s.prop23 = s.getQueryParam('bcd');
	s.eVar23 = s.prop23 ? 'D=c23' : null;
	/* prop24/eVar24 小業種01 */
	s.prop24 = s.getQueryParam('bcd01');
	s.eVar24 = s.prop24 ? 'D=c24' : null;
	/* prop25/eVar25 企業名 */
	s.prop25 = s.getQueryParam('companyname');
	s.eVar25 = s.prop25 ? 'D=c25' : null;
	/* prop26・prop27/eVar26・eVar27 先輩検索：検索軸・検索条件 */
	if (location.pathname.match(/\/2016\/search\/senior\/result\/?(.*)/)) {
		if (tempParamValue = s.getQueryParam('enter')) {
			s.prop26 = 'enter';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index1')) {
			s.prop26 = 'index1';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index2')) {
			s.prop26 = 'index2';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index3')) {
			s.prop26 = 'index3';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index4')) {
			s.prop26 = 'index4';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index5')) {
			s.prop26 = 'index5';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('index6')) {
			s.prop26 = 'index6';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('income')) {
			s.prop26 = 'income';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
		} else if (tempParamValue = s.getQueryParam('age')) {
			s.prop26 = 'age';
			s.eVar26 = s.prop26 ? 'D=c26' : null;
			s.prop27 = tempParamValue;
			s.eVar27 = s.prop27 ? 'D=c27' : null;
	...
	s.prop28 = s.getQueryParam('featureCd');
	s.eVar28 = s.prop28 ? 'D=c28' : null;
	/* prop29/eVar29 ブックマーク：顧客コード */
	if (location.pathname.match(/\/2016\/mypage\/bookmark\/register\/complete\/?(.*)/)) {
		s.prop29 = s.getQueryParam('kokyakuCd');
		s.eVar29 = s.prop29 ? 'D=c29' : null;
	}
  /* prop30/eVar30 フリーワード */
  try {
      s.prop30 = s.getQueryParam("freeword", "", decodeURIComponent(location.search));
      s.eVar30 = s.prop30 ? "D=c30" : "";
  } catch (e) {}

  /* prop31/eVar31 職種 */
  s.prop31 = s.getQueryParam("job");
  s.eVar31 = s.prop31 ? "D=c31" : "";

  /* prop32/eVar32 職種01 */
  s.prop32 = s.getQueryParam("job01");
  ...
  s.prop35 = s.pageName.replace(/:r[0-9]+:/, ":");
  s.eVar35 = s.prop35 ? "D=c35" : "";
	...
	*****************************************/
	...
	if ( !s.SetpdcFlag ) {
		s.SetpdcFlag = true;
		if ( !s.prop19 ) s.prop19 = s.getQueryParam('pdc');
		if ( s.prop19 ) {
			var tmp_array = s.prop19.match(/^[^_]+_[^_]+_[^_]+_(r[0-9]{9})$/);
			if( !tmp_array) { tmp_array = s.prop19.match(/^[^_]+_[^_]+_[^_]+_[^_]+_(r[0-9]{9})$/)};
			if ( tmp_array ) {
				s.sc_set_pdc_c('pdc_c_2016', tmp_array[1], '/2016/');
			} else if ( s.prop19 == 'dummy' ) {
				s.sc_remove_pdc_c('pdc_c_2016', '/2016/');
			} else {
				s.sc_set_pdc_c('pdc_c_2016', s.prop19, '/2016/');
			}
			if ( !s.eVar19 ) s.eVar19 = 'D=c19';
		} else {
			var pdc_c = s.sc_get_pdc_c('pdc_c_2016');
			if ( pdc_c && s.products ) {
				var is_cal = false;
				var is_match = false;
				...
				for ( var i=0; i<tmp_array.length; i++ ) {
					...
					if ( tmp_array2.length > 1 && tmp_array2[1].match(/^r[0-9]{9}$/) ) {
						is_cal = true;
				...
				if ( is_cal ) {
					if ( !is_match ) {
						s.prop19 = 'dummy';
						s.eVar19 = 'D=c19';
						s.sc_remove_pdc_c('pdc_c_2015', '/2015/');
					}
				}
			}
		}
